...
pushd git-kubo-release/manifests
...
do
  if [[ $release == kubo* ]]; then
    ...
    RELEASE_VERSION="$(bosh int git-kubo-release/manifests/cfcr.yml -o git-kubo-release/manifests/ops-files/non-precompiled-releases.yml --path=/releases/name=${release}/version)"
  fi
