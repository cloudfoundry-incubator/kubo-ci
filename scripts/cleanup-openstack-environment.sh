...
delete_vms() {
  network_id=$(bosh int "$ENV_FILE" --path='/net_id')
  ...
  internal_ip=$(bosh int "$ENV_FILE" --path='/internal_ip')
  ...
  do
    volume_names=$(openstack server show "${server_name}" -f yaml | bosh int --path /volumes_attached - | cut -d "'" -f2)
    if [ ! "${volume_names}" == "" ]; then
      arr=( $volume_names )
      for volume in "${arr[@]}"; do
        openstack server remove volume "${server_name}" "${volume}"

        if timeout 120 /bin/bash <<EOF
          until openstack volume show "${volume}" -f yaml | bosh int --path /status - | grep "available"; do
            sleep 2
          done
EOF
        then
...
OS_REGION_NAME=$(bosh int "$ENV_FILE" --path='/region')
OS_PROJECT_NAME=$(bosh int "$ENV_FILE" --path='/openstack_project')
OS_PROJECT_ID=$(bosh int "$ENV_FILE" --path='/openstack_project_id')
OS_PASSWORD=$(bosh int "$ENV_FILE" --path='/openstack_password')
OS_AUTH_URL=$(bosh int "$ENV_FILE" --path='/auth_url')
OS_USERNAME=$(bosh int "$ENV_FILE" --path='/openstack_username')
OS_USER_DOMAIN_NAME=$(bosh int "$ENV_FILE" --path='/openstack_domain')
