...
var _ = Describe("Kubectl", func() {
	...
	It("Should be able to run kubectl commands within pod", func() {
		...
		session := kubectl.StartKubectlCommand("run", podName, "--image", "pcfkubo/cfcr-nginx-bionic:latest", "--restart=Never", "--image-pull-policy=Always", "-ti", "--rm", "--", "kubectl", "get", "services")
		session.Wait(120)
