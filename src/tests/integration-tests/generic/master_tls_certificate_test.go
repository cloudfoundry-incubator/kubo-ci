...
var _ = Describe("MasterTlsCertificate", func() {
	...
	DescribeTable("hostnames", func(hostname string) {
		...
		session := kubectl.StartKubectlCommandInNamespace("default", "run", "test-master-cert-via-curl-"+GenerateRandomUUID(), "--image=gcr.io/cf-pks-golf/tutum/curl", "--restart=Never", "-ti", "--rm", "--", "curl", url, "--cacert", "/var/run/secrets/kubernetes.io/serviceaccount/ca.crt")
