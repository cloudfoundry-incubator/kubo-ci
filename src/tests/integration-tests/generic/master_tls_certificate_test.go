...
var _ = Describe("MasterTlsCertificate", func() {
	...
	DescribeTable("hostnames", func(hostname string) {
		...
		session := kubectl.RunKubectlCommandInNamespace("default", "run", "test-master-cert-via-curl-"+GenerateRandomUUID(), "--image=appropriate/curl", "--restart=Never", "-ti", "--rm", "--", "curl", url, "--cacert", "/var/run/secrets/kubernetes.io/serviceaccount/ca.crt")
